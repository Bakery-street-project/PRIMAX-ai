...
function selectBackend(): { baseUrl: string; apiKey: string; model: string } | null {
  ...
  if (nimKey) return { baseUrl: nimUrl, apiKey: nimKey, model: nimModel };
...
Deno.serve(async (req) => {
  ...
  // Parse request
  let body: { prompt: string; context?: string; projectId?: string };
  try {
    body = await req.json();
  } catch {
  ...
  if (body.projectId) {
    const { data: project } = await supabase
      .from("projects")
      .select("name, slug, status, metadata")
  ...
  if (!backend) {
    return json({ error: "No LLM backend configured. Set NIM_API_KEY, GEMINI_API_KEY, or OPENAI_API_KEY." }, 503);
  }

  const messages = [
    ...(dbContext ? [{ role: "user", content: `Context:\n${dbContext}` }, { role: "assistant", content: "Understood. Ready to help." }] : []),
  ...
  try {
    ...
    return json({ error: "LLM request failed", detail: err.message }, 502);
  }
  ...
  if (!llmRes.ok) {
    ...
    return json({ error: "LLM error", detail: errText, status: llmRes.status }, 502);
  }
  ...
  await supabase.from("agent_runs").insert({
    ...
    task_type: "chat",
    input: { prompt: body.prompt, projectId: body.projectId ?? null },
    output: { answer },
    user_id: user.id,
    meta_json: { model: backend.model, latency_ms: latencyMs },
  }).then(({ error }) => {
    if (error) console.warn("agent_runs insert failed:", error.message);
  });

  return json({ answer, model: backend.model, latency_ms: latencyMs });
});
