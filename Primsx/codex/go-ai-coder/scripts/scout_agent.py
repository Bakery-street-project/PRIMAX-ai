...
def fetch_github_trending():
    ...
    try:
        ...
        r.raise_for_status()
        return [item["html_url"] for item in r.json()["items"]]
...
def fetch_arxiv_ai_papers():
    ...
    try:
        ...
        return [
            l.split("<title>")[1].split("</title>")[0].strip()
            for l in r.text.split("<entry>")
            if "<title>" in l
        ]
    except Exception as e:
...
if __name__ == "__main__":
    ...
    arxiv = fetch_arxiv_ai_papers()
    log_to_md(topics, gh, arxiv)
    print("[Scout] Updated research log.")
