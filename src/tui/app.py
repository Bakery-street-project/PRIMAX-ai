...
from textual.widgets import Header, Footer, Static, Input, Button, Log
        ...
        try:
            ...
            for chunk in client.chat(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are PRIMAX-DRAGON, a wise AI assistant.",
                    },
            ...
            try:
                ...
                stdout, stderr = proc.communicate(
                    input=full_prompt, timeout=OLLAMA_GENERATION_TIMEOUT
                )
                self.log_message(
                    "PRIMAX",
                    stdout.strip() if proc.returncode == 0 else f"Error: {stderr}",
                )
            except subprocess.TimeoutExpired:
                self.log_message(
                    "PRIMAX", f"Error: Query timeout after {OLLAMA_GENERATION_TIMEOUT}s"
                )
        except Exception as e:
